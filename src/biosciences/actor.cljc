(ns biosciences.actor
  "FieldActor — the ISCO-08 2131 field/lab research support actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one field/lab operation request
  ...
  The unconditional invariant: the FieldAdvisor can never
  directly commit a record or dispatch a robot action the
  BiologyGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [biosciences.advisor :as advisor]
            [biosciences.governor :as governor]
            [biosciences.store :as store]))

(defn build-graph
  "Build a compiled FieldActor graph. `store` implements
  `biosciences.store/Store`. `advisor` implements
  `biosciences.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one field/lab operation request to completion or interrupt.
