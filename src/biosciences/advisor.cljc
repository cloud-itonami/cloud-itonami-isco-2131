(ns biosciences.advisor
  "FieldAdvisor — proposes a field/lab research operation (analyze a
  specimen, draft an ecological report, flag a species anomaly, request
  field equipment time) for a registered research project. The advisor
  ...
  has no notion of project provenance or field-safety risk;
  `biosciences.governor` is the independent system that decides whether
  the proposal may proceed, per the itonami actor pattern.

  A proposal is a map:
    {:op :analyze-specimen-data|:draft-report|:flag-species-anomaly
        |:request-field-equipment
...
(defn- infer
  ...
  finalized? or significant-finding? for testing edge cases."
  ...
  (cond-> {:op op
    ...
    (:significant-finding? request) (assoc :significant-finding? true)
    (:anomaly-type request) (assoc :anomaly-type (:anomaly-type request))))
...
(def ^:private system-prompt
  "You are a field research advisor for biology and ecology. Given a
   research operation request, propose an :op, an honest :confidence
  ...
  (reify Advisor
    (-advise [_ _store request]
      (let [msgs [{:role :system :content system-prompt}
                  {:role :user :content (str "research operation request: " (pr-str request))}]
